...
//进行绘制的各种函数
    ...
    function addMap(){
            ...
            state.selectAll("path")
                    ...
                    .on("click",function(d){
                            if(d.properties.hasData==1) Showmessage(d)
                    })
...
//结束绘制，开始各种函数了
    ...
    function Highlight(temp_range){
            ...
            d3.select(".state")
                ...
                .attr("fill",function(d){
                    //console.log(d)
                    //console.log(2333)
                    if(selected_dataset!="ASF"){
                        if(d.properties.stage==temp_range.id) return color(d.properties.stage)
                            else return
                    }
    ...
    function change_to_timerange(year_0,year_1){
        ...
        d3.selectAll(".overview_country")
            .text(function(){
                            ...
                            if(temp_lan=="english") return "Countries: "+t_country
                                else return "国家: "+t_country
                        })
        d3.selectAll(".overview_num")
            .text(function(){
                            ...
                            if(temp_lan=="english") return "Confirmed: "+t_num
                                else return "确诊: "+t_num
                        })
        d3.selectAll(".country_num_text")
            .text(function(d){
                if(is_data2axis==0){
    ...
    function Showmessage(temp_country){
        ...
        let detail_text=""
                    if(is_data2axis==0){
                        if(temp_lan=="chinese"){
                            detail_text+=("确诊:"+temp_country.properties.main_data[axis_end.toString()]+" ")
                            detail_text+=("治愈:"+temp_country.properties.cure_data[axis_end.toString()]+" ")                      
                            detail_text+=("死亡:"+temp_country.properties.death_data[axis_end.toString()]+" ")
                        }
                        else{
